...
use Kalnoy\Cruddy\Service\S3ThumbnailFactory;
...
class CruddyController extends Controller {
    ...
    public function thumb(Request $input, ThumbnailFactory $factory)
    {
        ...
        // $adjustedSrc = public_path($src);
        $adjustedSrc = S3Cache::getUrlFor($src);
        // throw new Exception('adjustedSrc: ' . $adjustedSrc, 401);
        
        try
        {
            // return $factory->make(public_path($src), $width, $height)->response();
            return $factory->make($adjustedSrc, $width, $height)->response();
        }
        ...
        catch (Exception $e)
        {
            ...
            die();
            return response(json_encode($e), 404);
