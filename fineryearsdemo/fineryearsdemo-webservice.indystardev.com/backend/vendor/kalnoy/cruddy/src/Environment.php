...
class Environment
{
    ...
    public function data()
    {
        return [
            ...
            's3RootUrl'     => config('s3.bucketUrl'),
            's3ImageCache'  => config('s3.imageCache'),
            's3Bucket'      => config('s3.s3Bucket'),
            'uri_prefix'    => config('cruddy.uri_prefix', ''),
            'loginUrl'      => config('cruddy.loginUrl', ''),
            'useDateTimePicker'     => config('cruddy.useDateTimePicker', ''),
            'dateTimePickerOptions' => config('cruddy.dateTimePickerOptions', ''),
            'closeAfterSave'        => config('cruddy.closeAfterSave'),
        ];
    }
