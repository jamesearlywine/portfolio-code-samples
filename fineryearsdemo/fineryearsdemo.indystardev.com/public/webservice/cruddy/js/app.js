(function() {
  ...
  Cruddy.Inputs.ImageList = (function(_super) {
    ...
    ImageList.prototype.renderImage = function(item) {
      ...
      return "<a href=\"" + (isFile ? item.data || "#" : Cruddy.s3RootUrl + '' + Cruddy.s3ImageCache + '/' + item) + "\" class=\"img-wrap\" data-trigger=\"fancybox\">\n    <img src=\"" + image + "\" " + (isFile ? "id='" + item.cid + "'" : "") + ">\n</a>";
    };
  ...
  Cruddy.Inputs.DateTime = (function(_super) {
    ...
    DateTime.prototype.initialize = function(options) {
      ...
      var datetime = DateTime.__super__.initialize.apply(this, arguments);
      
      /*
      Cruddy.useDateTimePicker = true;
      Cruddy.dateTimePickerOptions = {
        inline: true,
        sideBySide: true,
      ...
      */
      if (Cruddy.useDateTimePicker) {
        setTimeout(function() {
          this.datetime.$el.datetimepicker(this.Cruddy.dateTimePickerOptions);  
          this.datetime.$el.on('dp.change', function(e) {
            var value = this.datetime.$el.val();
            var timestamp = parseInt(moment(value, Cruddy.dateTimePickerOptions.format).format('X'), 10);
            this.datetime.applyChanges(timestamp, true);
            datetime.$el.trigger('change');
          }.bind({datetime: datetime}));
        }.bind({datetime: datetime, Cruddy: Cruddy}), 200);
      }
    ...
    DateTime.prototype.applyChanges = function(value, external) {
      // console.log('applying changes, value: ', value, ' external: ', external);
  ...
  Cruddy.Fields.BaseView = (function(_super) {
    ...
    BaseView.prototype.errorTemplate = function() {
      return "<span class=\"help-block error\" id=\"" + (this.componentId("error")) + "\"></span>";
    };
  ...
  Cruddy.Fields.BaseDateTime = (function(_super) {
    ...
    BaseDateTime.prototype.createEditableInput = function(model, inputId) {
      var datetime = new Cruddy.Inputs.DateTime({
        model: model,
        key: this.id,
  ...
  Cruddy.formatters.Image = (function(_super) {
    ...
    Image.prototype.format = function(value) {
      ...
      return "<a href=\"" + (Cruddy.s3RootUrl + "" + Cruddy.s3ImageCache + '/' + value) + "\" data-trigger=\"fancybox\">\n    <img src=\"" + (thumb(value, this.options.width, this.options.height)) + "\" " + (this.options.width ? " width=" + this.options.width : "") + " " + (this.options.height ? " height=" + this.options.height : "") + " alt=\"" + (_.escape(value)) + "\">\n</a>";
    };
  ...
  Cruddy.Entity.Form = (function(_super) {
    ...
    Form.prototype.handleModelDestroyEvent = function() {
      ...
      console.log('handleModelDestroyEvent(), triggering destroyed');
      this.trigger("destroyed", this.model);
      if (Cruddy.closeAfterSave) {
        setTimeout(function() {
          this.closeForm();
          $('.ep-btn-refresh').trigger('click');
        }.bind(this), 100);
      }
    ...
    Form.prototype.save = function(options) {
      ...
      var saveOptions =  $.extend({}, this.saveOptions, options);
      isNew = this.model.isNew();
      window.last_this = this;
      
      this.setupRequest(this.model.save(null, saveOptions));
  ...
  App = (function(_super) {
    ...
    App.prototype.handleAjaxError = function(xhr) {
      ...
      if (xhr.status === VALIDATION_FAILED_CODE) {
    	...
    	// if failed due to not authorized
      if (xhr.status === 401) {
        // redirect to Cruddy.loginUrl
        window.isRedirectingToAuth = true;
        window.location.href = Cruddy.loginUrl;
      }
    ...
    App.prototype.pageUnloadConfirmationMessage = function() {
      var _ref;
      return ((_ref = this.entityView) != null && !window.isRedirectingToAuth) ? _ref.pageUnloadConfirmationMessage() : void 0;
    };
  ...
  Router = (function(_super) {
    ...
    Router.prototype.updateQuery = function(query, options) {
      ...
      if ((qs = query.toString()) !== this.query.toString()) {
        ...
        var prefix = (Cruddy.uri_prefix !== null)
          ? Cruddy.uri_prefix.replace(/\/+$/,'') + "/"
          : ''    
        ;
        uri = "/" + prefix + Cruddy.uri + "/";
  ...
  $(function() {
    ...
    var prefix = (Cruddy.uri_prefix !== null && Cruddy.uri_prefix.trim() !== '')
      ? Cruddy.uri_prefix.replace(/\/+$/,'') + "/"
      : ''    
    ;
    return Backbone.history.start({
      root: Cruddy.uri_prefix + Cruddy.getHistoryRoot(),
      pushState: true,
      hashChange: false
    });
  });
